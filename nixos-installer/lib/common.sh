...
check_deps() {
    local missing=()
    for cmd in parted mkfs.fat mkfs.ext4 mkswap lsblk lspci nixos-generate-config nixos-install; do
        command -v "$cmd" &>/dev/null || missing+=("$cmd")
    done
    if (( ${#missing[@]} > 0 )); then
        die "Missing required commands: ${missing[*]}"
    fi
    # openssl is optional — nix-shell fallback is used if absent
    if ! command -v openssl &>/dev/null; then
        log_warn "openssl not found — will use 'nix-shell -p openssl' for password hashing"
    fi
}
